...
# 👇 Para que Airflow encuentre tus scripts
...
with DAG(
    ...
    def branch_drift(**kwargs):
        ...
        if not model_exists:
            print("⚠️ No existe modelo entrenado. Procediendo a entrenar.")
            return 'train_model'

        if drift_detected:
            print("⚠️ Drift detectado. Procediendo a entrenar.")
            return 'train_model'
        else:
    ...
    # -----------------------------------------
    # INTEGRATE BATCH PARQUET TASK
    # -----------------------------------------

    def integrate_batch_parquet_task(**kwargs):
        # Obtener nombre del archivo desde Variable Airflow
        batch_filename = Variable.get("NEW_BATCH_FILENAME")

        # 1. Leer nuevo archivo parquet
        ...
        # 8. Crear parquet para la próxima semana
